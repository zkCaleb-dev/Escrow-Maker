 ...
 */
function signTransaction(unsignedXdr, secretKey) {
  try {
    // 1. Convertir el XDR (base64) en TransactionEnvelope
    //    A partir de ahí, creamos un objeto Transaction con la red TESTNET.
    ...
    // 2. Obtener el Keypair a partir de la secretKey proporcionada
    let keypair;
    try {
    ...
    // 4. Serializar de nuevo a XDR en base64 y devolver
    return transaction.toXDR('base64');
  } catch (err) {
    // Si cae aquí, devolvemos el mensaje de error para que la llamada lo capture
